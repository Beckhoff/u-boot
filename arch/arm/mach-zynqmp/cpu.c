...
static void early_enable_caches(void)
{
	if (CONFIG_IS_ENABLED(SYS_ICACHE_OFF) ||
	    ...
	    CONFIG_IS_ENABLED(CONFIG_ZYNQMP_NO_DDR))
		return;

	icache_enable();

	/* Use by default the maximum available DRAM size of the first/low memory region. */
	zynqmp_mem_map[ZYNQMP_MEM_MAP_USED].virt =
		EARLY_CACHE_LOW_MEM_REG_VIRT_ADDRESS;
	zynqmp_mem_map[ZYNQMP_MEM_MAP_USED].phys =
		EARLY_CACHE_LOW_MEM_REG_PHYS_ADDRESS;
	zynqmp_mem_map[ZYNQMP_MEM_MAP_USED].size = EARLY_CACHE_LOW_MEM_REG_SIZE;
	zynqmp_mem_map[ZYNQMP_MEM_MAP_USED].attrs =
